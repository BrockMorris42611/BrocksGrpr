        ...
        layout.findViewById<Button>(R.id.createAccountButton)
            .setOnClickListener{
                ...
                if (password.equals(passwordConfirm)) {
                    ...
                    Helper.api.createAccount(requireContext(),
                        ...
                        object : Helper.api.Response {
                            override fun processResponse(response: JSONObject) {
                                if (Helper.api.isSuccess(response)) {
                                    ...
                                    Helper.api.registerFCMtoken(requireContext(),
                                        ...
                                        FirebaseMessaging.getInstance().token.toString(), object: Helper.api.Response {
                                            override fun processResponse(response: JSONObject) {
                                                if (Helper.api.isSuccess(response)) {
                                                    ...
                                                    Toast.makeText(requireContext(), Helper.api.getErrorMessage(response),
                                                        Toast.LENGTH_SHORT).show()
                                                }
                                            }
                                        })
                                    Helper.user.saveCreatorFlag(requireContext(), false)
                                    goToDashboard()
                                } else {
                                    Toast.makeText(requireContext(), Helper.api.getErrorMessage(response), Toast.LENGTH_SHORT
                                    ).show()
                                }
                            }
                        }
